---
title: "COVID-19 among US counties"
author: "Chris Berg"
...
us_counties_map$NAME[which(us_counties_map$GEOID == "36061")] = "New York"

most_recent_data = 
  readRDS( paste0("data/export/us-county-data-updated-" , recent_date , ".rds" ,sep = "")
  ) %>%
  filter( last_update == recent_date & fips < 60000 ) %>%
...
Different counties have different proportions of confirmed cases which have died. This can be due to not testing enough, and so only picking-up the most severe cases which are most-likely to end in fatality, but this is only one of many causes for the variation we see. Below is the entire distribution of case fatality ratios that we see today among US counties.

```{r histogram, echo=F, warning=F}
library(hrbrthemes)
