...
us_daily_data <- 
  ...
  mutate( new_cases = cases - dplyr::lag(cases, order_by = date) ,
          avg_new_cases = 
            ...
            dplyr::lag(new_cases, order_by = date, n = 6))*(1/7) ,
...
us_counties = 
  merge( most_recent_data , us_counties_map ) %>%
  st_as_sf()
...
dom_min <- ifelse(min(us_counties$avg_new_cases)<0,
                  abs(floor(min(us_counties$avg_new_cases))),
                  1)

col_pal <- colorNumeric(palette = "YlOrRd", domain = log(dom_min+us_counties$avg_new_cases) )
...
county_plot = 
  ...
  addPolygons(
    ...
    smoothFactor = 0,
    fillOpacity = 0.7,
    color = ~col_pal(log(dom_min+avg_new_cases))
  ) %>%
  addLegend(
    "bottomright", 
    pal = col_pal, 
    values = ~log(dom_min+avg_new_cases),
